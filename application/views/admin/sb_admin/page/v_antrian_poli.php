...
<form action="<?php echo base_url('admin/Antrian_poli/save') ?>" method="post">
	...
	<div class="row">
		<div class="col-md-2">
			<label>Nomor Antrian Terakhir</label>
		</div>
		<div class="col-md-5">
			<?php foreach ($getPoli as $poli) { ?>
				<h4><?php echo $poli->nama_poli; ?>: <?php echo isset($lastQueueNumbers[$poli->id_poli]) ? $lastQueueNumbers[$poli->id_poli] : 'Tidak Ada Pasien'; ?></h4>
			<?php } ?>
		</div>
	</div>
